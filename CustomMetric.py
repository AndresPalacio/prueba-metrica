...
def memProcessChromeDrive():
    return len([p.info['pid'] for p in psutil.process_iter(attrs=['pid','name','create_time'])
             if 'chromedriver' in p.info['name']
              if(datetime.datetime.now() - datetime.datetime.fromtimestamp(p.info['create_time'])).seconds //3600 >= 1])
...
if __name__ == '__main__':
    ...
    appendMetrics(currMetrics, dimensions, Name='ProcessChromedriver', Value=memProcessChromeDrive(), Unit='Value')
